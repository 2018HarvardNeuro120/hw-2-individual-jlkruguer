...
for i = 1:nTrials
    tStart = edges(i);
    tEnd = edges(i+1);
    exp(:,i) = psth(spikesExp,tStart,tEnd,binWidth); 
    con(:,i) = psth(spikesCon,tStart,tEnd,binWidth);
end

t = 0:binWidth:1/6;
plot(t*1000,mean(exp,2)/N_exp,t*1000,mean(con,2)/N_control,'linewidth',2)
